...
Matchings_predictor::Matchings_predictor(Patient &patient,
                                         std::string simulation_id)
    : patient(patient) {
  // TODO: for the moment I am just considering tumor mirnas and tumor clusters.
  // In the future I should consider the difference between normal and tumor
  // expressions. note that these are copies, we do not want to modify the
  ...
  */
  ...
  std::string cmd = "mkdir -p ./data/patients/" + this->patient.case_id +
                    "/matchings_predictor_output";
...
Matchings_predictor::Matchings_predictor(const Matchings_predictor &obj)
    : patient(obj.patient) {
  ...
  this->p_j_downregulated_given_c_bound_values =
      obj.p_j_downregulated_given_c_bound_values;
  this->p_j_downregulated_values = obj.p_j_downregulated_values;
  this->simulation_id = obj.simulation_id;
  this->lambda = obj.lambda;
  this->lambda_adjustment_due_to_mirna_perturbation =
      obj.lambda_adjustment_due_to_mirna_perturbation;
  this->lambda_adjustment_due_to_gene_perturbation =
      obj.lambda_adjustment_due_to_gene_perturbation;
  this->genes_skipped_by_the_distance_based_predictor =
      obj.genes_skipped_by_the_distance_based_predictor;
}
...
void swap(Matchings_predictor &obj1, Matchings_predictor &obj2) {
  ...
  std::swap(obj1.p_j_downregulated_given_c_bound_values,
            obj2.p_j_downregulated_given_c_bound_values);
  ...
  std::swap(obj1.lambda_adjustment_due_to_mirna_perturbation,
            obj2.lambda_adjustment_due_to_mirna_perturbation);
  std::swap(obj1.lambda_adjustment_due_to_gene_perturbation,
            obj2.lambda_adjustment_due_to_gene_perturbation);
  std::swap(obj1.genes_skipped_by_the_distance_based_predictor,
            obj2.genes_skipped_by_the_distance_based_predictor);
}
...
std::string Matchings_predictor::get_output_path() {
  std::string path = "./data/patients/" + this->patient.case_id +
                     "/matchings_predictor_output/" + simulation_id + "/";
  return path;
}
  ...
  for (auto &mirna : this->mirna_profile) {
    ...
    for (auto &e : this->cluster_profile) {
      ...
      for (auto &site : cluster->sites) {
        ...
        if (this->patient.interaction_graph.mirna_site_arcs.find(p) !=
            this->patient.interaction_graph.mirna_site_arcs.end()) {
          matchings++;
        }
      }
...
void Matchings_predictor::compute() {
  ...
  if (!logging &&
      (export_mirna_expression_profile || export_cluster_expression_profile ||
       export_interaction_matrix || export_partial_predicted_downregulation)) {
  ...
  if (logging) {
    std::string filename0 =
        this->get_output_path() + "matchings_prediction.tsv";
    out0.open(filename0);
    ...
    for (t = 0; t < max_steps;) {
      if (rank == 0) {
        ...
        if (current_percentage > latest_percentage) {
          ...
          std::cout << "t/max_steps: " << t << "/" << max_steps << " = "
                    << current_percentage << "%\n";
          if (logging) {
            if (export_mirna_expression_profile) {
              ...
              mirna_log_filename << mirna_log_filename_prefix
                                 << std::setfill('0') << std::setw(6) << t
                                 << ".tsv";
            ...
            if (export_cluster_expression_profile) {
              ...
              cluster_log_filename << cluster_log_filename_prefix
                                   << std::setfill('0') << std::setw(6) << t
                                   << ".tsv";
        ...
        if (logging) {
          ...
          double adjusted_avg_mirna_level =
              avg_mirna_level / cumulative_scaling;
          double adjusted_avg_cluster_level =
              avg_cluster_level / cumulative_scaling;
        ...
        loop_size =
            this->patient.interaction_graph.gene_to_clusters_arcs.size();
      }
#ifndef OMP_TEMPORARILY_DISABLED
#pragma omp barrier
#endif
      ...
      auto rank_r_ic_values =
          std::unordered_map<std::pair<Mirna_id, Cluster *>, double>();
      auto rank_r_ijk_values =
          std::unordered_map<std::pair<Mirna_id, Site *>, double>();
      ...
      for (; i < loop_size;) {
        ...
        for (Cluster *cluster : clusters) {
          for (Site *site : cluster->sites) {
            for (const Mirna_id &mirna_id :
                 this->patient.interaction_graph.site_to_mirnas_arcs.at(site)) {
              ...
              double exchange = h * mirna_value * cluster_value /
                                number_of_sites_for_the_mirna_in_the_cluster;
              ...
              if (exchange < 0) {
                ...
                if (abs(exchange) < Global_parameters::epsilon) {
                  if (!warning_already_presented) {
                    ...
                    std::cerr << "warning: exchange = " << exchange
                              << ", silencing further warnings like this\n";
                  }
                } else {
              ...
              if (cumulative_scaling < 1 - Global_parameters::epsilon) {
                std::cerr << "error: cumulative_scaling = "
                          << cumulative_scaling << "\n";
                exit(1);
              }
              rank_mirna_total_exchange +=
                  (mirna_lambda * exchange / cumulative_scaling);
              rank_cluster_total_exchange +=
                  (cluster_lambda * exchange / cumulative_scaling);
              ...
              rank_r_ijk_values[ijk] +=
                  cluster_lambda * exchange / cumulative_scaling;
            }
          }
        }

        for (unsigned long long j = 0; j < my_num_threads && i < loop_size;
             j++) {
          it++;
          i++;
        }
...
void Matchings_predictor::compute_probabilities() {
  ...
  for (auto &e : this->r_ic_values) {
    ...
    // this->p_j_downregulated_given_c_bound_values
    if (this->sum_of_r_ijk_for_cluster.find(cluster) ==
        this->sum_of_r_ijk_for_cluster.end()) {
      this->sum_of_r_ijk_for_cluster[cluster] = 0;
    }
    this->sum_of_r_ijk_for_cluster[cluster] =
        this->sum_of_r_ijk_for_cluster.at(cluster) + value;
    ...
    this->p_c_bound_values[cluster] =
        this->p_c_bound_values.at(cluster) +
        value / (this->original_cluster_profile.at(cluster));
  }
  ...
  for (auto &e : this->r_ic_values) {
    ...
    std::unordered_map<Site *, std::list<std::pair<Mirna_id, Site *>>>
        just_added_by_site;
    for (auto &site : cluster->sites) {
    ...
    if (just_added_by_site.size() == 0) {
      std::cerr << "error: just_added_by_site.size() = "
                << just_added_by_site.size() << "\n";
      exit(1);
    }
    for (auto &e : just_added_by_site) {
      ...
      for (auto &p : e.second) {
        ...
        if (this->p_j_downregulated_given_c_bound_values.find(key0) ==
            this->p_j_downregulated_given_c_bound_values.end()) {
          this->p_j_downregulated_given_c_bound_values[key0] = 0;
        }
        double r_ijk = this->r_ijk_values.at(p);
        Mirna_site_arc &mirna_site_arc =
            this->patient.interaction_graph.mirna_site_arcs.at(p);
        double context_score = mirna_site_arc.context_score;
        double probability_of_downregulation = 1 - std::pow(2, context_score);
        if (std::abs(sum_of_r_ijk_for_cluster.at(cluster)) <
          ...
          if (std::abs(sum_of_r_ijk_for_cluster.at(cluster)) <
            ...
            std::cerr
                << "error: division by zero. Do you really need to multiply by "
          ...
          double to_add = r_ijk * probability_of_downregulation /
                          sum_of_r_ijk_for_cluster.at(cluster);
          // double to_add = r_ijk * probability_of_downregulation /
          // p_c_bound_values.at(cluster);
          this->p_j_downregulated_given_c_bound_values[key0] =
              this->p_j_downregulated_given_c_bound_values.at(key0) + to_add;
        }
      }
    }
  }
...
// #define TEST_THE_ITERATIVE_ALGORITHM
  ...
  for (auto &e : this->patient.interaction_graph.gene_to_clusters_arcs) {
    ...
    max_number_of_clusters_per_gene =
        std::max(max_number_of_clusters_per_gene, clusters.size());
  }
#ifdef TEST_THE_ITERATIVE_ALGORITHM
  // begin debug code
  ...
  // end debug code
#endif
  double *p_j_downregulated_given_c_bound_values_flattened =
      new double[max_number_of_clusters_per_gene];
  double *p_c_bound_values_flattened =
      new double[max_number_of_clusters_per_gene];
  ...
  for (auto &e : this->patient.interaction_graph.gene_to_clusters_arcs) {
    ...
    int current_percentage =
        (int)(100 *
              ((double)(genes_debugged + genes_not_debugged) / total_genes));
    if (current_percentage > latest_percentage) {
      latest_percentage = current_percentage;
      std::cout << "genes_debugged/total_genes: "
                << genes_debugged + genes_not_debugged << "/" << total_genes
                << " = " << current_percentage << "%\n";
    }
    // end debug code
#endif
    ...
    for (Cluster *cluster : clusters) {
      ...
      p_j_downregulated_given_c_bound_values_flattened[i] =
          this->p_j_downregulated_given_c_bound_values.at(
              std::make_pair(gene_id, cluster));
    ...
    double sum = this->iteratively_compute_p_j_downregulated(
        p_j_downregulated_given_c_bound_values_flattened,
        p_c_bound_values_flattened, clusters.size());
    ...
    if (clusters.size() <= cluster_limit) {
      double debug_sum = 0;
      this->recusively_compute_p_j_downregulated(
          b, 0, clusters.size(), &debug_sum, 1, 1,
          p_j_downregulated_given_c_bound_values_flattened,
          p_c_bound_values_flattened);
      if (std::abs(sum - debug_sum) > Global_parameters::epsilon) {
        std::cerr << "error: sum = " << sum << ", debug_sum = " << debug_sum
                  << ", std::abs(sum - debug_sum) = "
                  << std::abs(sum - debug_sum) << "\n";
        std::cout << "describing the instance: clusters.size() = "
                  << clusters.size() << "\n";
        std::cout << "p_j_downregulated_given_c_bound_values_flattened, "
                     "p_c_bound_values_flattened\n";
        for (int j = 0; j < clusters.size(); j++) {
          std::cout << p_j_downregulated_given_c_bound_values_flattened[j]
                    << ", " << p_c_bound_values_flattened[j] << "\n";
        }
        exit(1);
      }
      genes_debugged++;
...
#ifdef TEST_THE_ITERATIVE_ALGORITHM
  std::cout << "genes_debugged/total_genes: " << genes_debugged << "/"
            << (genes_debugged + genes_not_debugged) << " = "
            << ((double)genes_debugged) / (genes_debugged + genes_not_debugged)
            << "\n";
  std::cout << "finished, ";
  Timer::stop();
  delete[] b;
 ...
 */
void Matchings_predictor::recusively_compute_p_j_downregulated(
    bool *b, int level, int max_level, double *sum,
    double p_j_downregulated_given_b, double p_b,
    double *p_j_downregulated_given_c_bound_values_flattened,
    double *p_c_bound_values_flattened) {
  if (level == max_level) {
    ...
    *sum += p_j_downregulated_given_b * p_b;
    ...
    double p_j_downregulated_given_c_bound =
        p_j_downregulated_given_c_bound_values_flattened[level];
    ...
    recusively_compute_p_j_downregulated(
        b, level + 1, max_level, sum, new_p_j_downregulated_given_b, new_p_b,
        p_j_downregulated_given_c_bound_values_flattened,
        p_c_bound_values_flattened);

    b[level] = 1;
    new_p_j_downregulated_given_b =
        p_j_downregulated_given_b * (1 - p_j_downregulated_given_c_bound);
...
/*
  ...
  clusters in a gene.
*/
double Matchings_predictor::iteratively_compute_p_j_downregulated(
    double *p_j_downregulated_given_c_bound_values_flattened,
    double *p_c_bound_values_flattened, int clusters_count) {
  double ratio_j_not_downregulated = 1.0;
  for (int i = 0; i < clusters_count; i++) {
    double newly_downregulated =
        ratio_j_not_downregulated * p_c_bound_values_flattened[i] *
        p_j_downregulated_given_c_bound_values_flattened[i];
...
void Matchings_predictor::export_probabilities() {
  ...
  filename =
      this->get_output_path() + "p_j_downregulated_given_c_bound_values.tsv";
  out.open(filename);
  out << ss.str();
  out.close();
...
void Matchings_predictor::export_p_j_downregulated(int filename_suffix) {
  ...
  filename << this->get_output_path()
           << "predicted_downregulation/p_j_downregulated_values_"
           << std::setfill('0') << std::setw(6) << filename_suffix << ".tsv";
