...
class Matchings_predictor
{
    ...
    inline void recusively_compute_p_j_downregulated_distance_based(
        char* b, int level, int max_level, double* sum, double p_j_downregulated_given_b, double p_b, Binding_flattened* bindings_flattened, double** distance_based_enhance_matrix, unsigned long latest_bound_level, unsigned long second_latest_bound_level);

public:
    double lambda;
    double relative_mirna_to_reads_conversion_factor;
    double relative_mirna_to_rpm_conversion_factor;
