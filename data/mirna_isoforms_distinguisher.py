...
def distinguish_mirna_isoforms(tissue: str):
	...
	if os.path.isfile(isoforms_filename):
		...
		print(f'{len(df_excluded)} mirna precursors exlcuded, {int(excluded_reads)}/{total_reads} = {round(excluded_reads/total_reads,7)} excluded reads')
		...
		if len(df_not_disambiguated) > 0:
			...
			print(f'unable to disambiguate {len(df_not_disambiguated)} mirna precursors, {not_disambiguated_reads}/{total_reads} = {round(not_disambiguated_reads/total_reads,7)} not disambiguated reads')
