...
** TODO look at the mirna expression profiles for other HeLa cells
** TODO look at the gene expression profiles
** TODO predict the gene down-regulation for the real data and compare the predictions with pairwise targetscan scores
** TODO in the case of gene expression profiles the spearman correlation is more appropriate since what matters is the log-fold change and not the absolute quantity. Anyway in this case the fact that the variance increases maybe should be taken into account
...
** TODO maybe I should use the Pearson correlation on the non transformed data, to reflect the fact that the most expressed mirnas are the most influential
