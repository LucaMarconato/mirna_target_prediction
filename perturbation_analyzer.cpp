                                ...
                                std::string output_name_suffix)
{
    ...
    simulation_id << (mirna_perturbation_type == Perturbation_type::Point_perturbation ? "p" : (mirna_perturbation_type == Perturbation_type::Gaussian_perturbation) ? "g" : "") << "_"
                  << (gene_perturbation_type == Perturbation_type::Point_perturbation ? "p" : (gene_perturbation_type == Perturbation_type::Gaussian_perturbation) ? "g" : "") << "_" << mirna_perturbation_target.string_id() << "_"
                  << gene_perturbation_target.string_id() << "_" << mirna_perturbation_extent.string_id() << "_" << gene_perturbation_extent.string_id() << "_" << output_name_suffix;
    this->perturb();
...
void Perturbation_analyzer::perturb()
{
    ...
    */
    ...
    matchings_predictor->relative_mirna_to_reads_conversion_factor = total_reads_after_perturbation;
    matchings_predictor->relative_mirna_to_rpm_conversion_factor = total_reads_after_perturbation / total_reads * 1000000;
    this->matchings_predictor->lambda *= lambda_adjustment_due_to_mirna_perturbation;
    this->matchings_predictor->lambda_adjustment_due_to_mirna_perturbation = lambda_adjustment_due_to_mirna_perturbation;
}
