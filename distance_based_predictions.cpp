        ...
        if(!skip_the_computation) {
            ...
            for(auto & current_distance_based_cluster : distance_based_clusters) {
                ...
                for(int k = 0; k < current_distance_based_cluster.size(); k++) {
                    ...
                    std::cout << "r_ijk = " << current_distance_based_cluster[k].r_ijk << "\n";
            ...
            if(std::abs(this->p_j_downregulated_values.at(gene_id) - total_p_j_downregulated) > Global_parameters::epsilon) {
                std::cout << "abs(this->p_j_downregulated_values.at(gene_id) - total_p_j_downregulated) = " << abs(this->p_j_downregulated_values.at(gene_id) - total_p_j_downregulated) << "\n";
...
void Matchings_predictor::recusively_compute_p_j_downregulated_distance_based(bool * b, int level, int max_level,
                                                                              ...
                                                                              bool the_current_cluster_is_free_to_bind)
{
    if(level == max_level) {
        ...
        *sum += p_j_downregulated_given_b * p_b;
        ...
        double r_ijk = bindings_flattened[level].r_ijk;
